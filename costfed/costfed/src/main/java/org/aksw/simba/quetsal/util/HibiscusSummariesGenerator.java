 ...
 *  @author saleem
 */
public class HibiscusSummariesGenerator {
	 ...
	 */
	public HibiscusSummariesGenerator(String location) throws IOException 
	{
	...
	public String makeWellFormed(String uri) {
		try {
			new TurtleParserFactory().getParser().parse(new StringReader("<" + uri + "> <http://example.com> <http://example.com> ."), "http://example.com");
			SimpleValueFactory factory = SimpleValueFactory.getInstance();
			IRI iri = factory.createIRI(uri);
			if (!iri.stringValue().equals(uri)) {
			...
			e.printStackTrace();
			System.out.println("Error involving: " + uri);
	...
	public void writeSbjAuthority(String predicate, String endpoint, String graph) throws RepositoryException, MalformedQueryException, QueryEvaluationException, IOException {
		...
		try{
			...
			while (res.hasNext()) 
			{
				...
				{
					try{
						String sbjAuth = makeWellFormed(sbjPrts[0]+"//"+sbjPrts[2]);
				...
				else
				{
					...
					if ((objPrts.length>1))
					{
						try{
							...
							String objAuth =makeWellFormed(objPrts[0]+"//"+objPrts[2]);
