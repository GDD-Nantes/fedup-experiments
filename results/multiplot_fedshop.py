...
df = df[df['engine'].str.contains(kept_approach)]
df = df[df['batch'] == kept_batch]


grouped_df = df.groupby(['query', 'engine', 'batch'])
...
header = ["query", "sparql 1.1", "costfed", "fedx", "semagrow", "fedup", "rsa_err", "costfed_err", "fedx_err", "semagrow_err", "fedup_err"]
    
with open(f'processed{kept_batch}.csv', 'w', newline='') as file:
